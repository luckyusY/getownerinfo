...
export default function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 shadow-lg shadow-black/10">
      {/* Announcement */}
      <div className="bg-[#0a1f44] px-3 py-1 text-center text-[11px] font-bold leading-[18px] text-white">
      ...
      {/* Utility bar */}
      <div className="hidden bg-[#0a55a4] text-white sm:block">
      ...
      <div className="bg-gradient-to-b from-[#1576c9] to-[#0c4f9c] text-white">
        <div className="mx-auto grid w-full max-w-6xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-2.5 sm:gap-5">
          <Link href="/" className="flex shrink-0 items-center gap-2" aria-label="getownerinfo home">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white text-base font-extrabold text-[#0c4f9c]">g</span>
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            {session ? (
              <Link href={dashboardPath} className="hidden items-center gap-1.5 text-sm font-bold text-white hover:text-[#ffcf57] sm:flex">
                <CircleUser className="h-6 w-6" />
                <span className="leading-tight text-left">My<br />Account</span>
                <ChevronDown className="h-4 w-4" />
              </Link>
            ) : (
              <Link href="/login" className="hidden items-center gap-1.5 text-sm font-bold text-white hover:text-[#ffcf57] sm:flex">
                <CircleUser className="h-6 w-6" />
                <span className="leading-tight text-left">Sign in<br />My Account</span>
                <ChevronDown className="h-4 w-4" />
              </Link>
            )}
            <Link href="/dashboard/buyer" aria-label="Saved listings" className="hidden text-white hover:text-[#ffcf57] sm:block">
